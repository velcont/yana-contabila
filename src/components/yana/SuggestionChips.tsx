...
export function SuggestionChips({ onSendMessage, onUpload, disabled, postAnalysis }: SuggestionChipsProps) {
  ...
  return (
    <div className="flex gap-2 py-2 overflow-x-auto flex-nowrap justify-start -mx-1 px-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden snap-x [mask-image:linear-gradient(to_right,transparent,black_4px,black_calc(100%-4px),transparent)]">
      {chips.map((chip) => {
